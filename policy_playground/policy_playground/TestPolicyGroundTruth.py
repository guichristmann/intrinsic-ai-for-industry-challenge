...
class TestPolicyGroundTruth(Policy):
    def __init__(self, parent_node):
        self._tip_x_error_integrator = 0.0
        self._tip_y_error_integrator = 0.0
        self._max_integrator_windup = 0.05
        self._task = None
        self._stop_tolerance = 0.005
    ...
    def _plug_to_port_distance(self, port_frame: str, cable_tip_frame: str) -> float:
        ...
        dz = port_tf.transform.translation.z - plug_tf.transform.translation.z
    ...
    def insert_cable(
        ...
        self.get_logger().info(
            f"TestPolicyGroundTruth.insert_cable() task: {task}, "
            f"stop_tolerance: {self._stop_tolerance}m"
        )
        self._task = task
        ...
        while True:
            try:
                dist = self._plug_to_port_distance(port_frame, cable_tip_frame)
            except TransformException as ex:
                self.get_logger().warn(f"TF lookup failed during distance check: {ex}")
                dist = float("inf")

            self.get_logger().info(
                f"z_offset: {z_offset:0.5}, plug-to-port distance: {dist:.4f}m"
            )

            if dist <= self._stop_tolerance:
                self.get_logger().info(
                    f"Within tolerance ({dist:.4f}m <= {self._stop_tolerance}m), stopping."
                )
                break

            if z_offset < -0.015:
                self.get_logger().info(
                    "Reached z_offset limit without meeting tolerance, stopping."
                )
                break
        ...
        self.get_logger().info("TestPolicyGroundTruth.insert_cable() exiting...")
        return True
